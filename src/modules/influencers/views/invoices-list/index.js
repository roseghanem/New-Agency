      ...
      return (
        <div className='d-flex'>
          <UncontrolledDropdown>
            ...
            <DropdownMenu right>
              <CanCall action='INFLUENCER_VIEW_INVOICE_PROFILE' id={`updateUser_${row.id}`}>
              ...
              {!row.is_paid && (
                  <CanCall action='INFLUENCER_PAY_INVOICE' id={`updateUser_${row.id}`}>
                    <DropdownItem className='w-100' onClick={e => _editBasicInfoModal(row)}>
                      <FileText size={15} />
                      <span className='align-middle ml-50'>Pay Invoice Manually</span>
                    </DropdownItem>
                  </CanCall>
              )}
            </DropdownMenu>
...
const tableActions = ['INFLUENCER_VIEW_INVOICE_PROFILE', 'INFLUENCER_PAY_INVOICE']
//************************************//
class UserList extends Component {
  ...
  render () {
    ...
    return (
      <Fragment>
        <Breadcrumbs breadCrumbMainTitle={''} breadCrumbTitle={'Invoices List'} breadCrumbParent='' breadCrumbActive='' ></Breadcrumbs>
        <Row>
          <Col sm='12'>
            <DataTable ref={(ref) => { this.dataTableRef = ref }} uri={'invoices'} columns={tableColumns(this.state, this._editBasicInfoModal, this._deleteUser, hasAction)} hasIndexing={true} hasFilter={true} defaultCollapse={true}/>
          </Col>
        </Row>
