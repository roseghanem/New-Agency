...
import {
  _addTier,
  _editTier, _getAllCampaignTypesWithQ, _getAllUserTypesWithQ
} from "../../redux/actions"
...
const BasicInfoModal = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    setValErrors({})
    data.campaign_type_id = data.campaign_type_id.value
    if (isEditAction) {
      _editTier(
    ...
    >
      <Form action='/' className='flex-grow-1 d-flex flex-column' onSubmit={handleSubmit(onSubmit)}>
        <ModalHeader toggle={_close} className='mb-1'>
          {isEditAction ? 'Edit Tier' : 'Add Tier'}
        </ModalHeader>
        <ModalBody className='flex-grow-1 pb-sm-0 pb-3 modal-body'>
          ...
          <FormGroup>
            <Label className='form-label' for='campaign_type_id'>
              Campaign Type
            </Label>
            <Controller
                as={AsyncSelect}
                ...
                defaultOptions
                cacheOptions
                loadOptions={_getAllCampaignTypesWithQ}
                className={classnames('react-select', { 'is-invalid': errors['campaign_type_id'] || _.get(valErrors, 'campaign_type_id') })}
                defaultValue={_.get(props, 'data.campaign_type_id') ? {label: _.get(props, 'data.campaign_type.name'), value: _.get(props, 'data.campaign_type.id')} : ''}
                rules={{
                  required: trans('user.validation.required')
                }}
            />
            <ErrorMessages valErrors={valErrors} errors={errors} name={'campaign_type_id'} />
          </FormGroup>
        </ModalBody>
