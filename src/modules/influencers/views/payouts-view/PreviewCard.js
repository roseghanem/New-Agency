...
import {FileUrl} from "../../utility/Utils"
        ...
        <div className='d-flex justify-content-between flex-md-row flex-column invoice-spacing mt-0'>
          ...
          <div className='mt-md-0 mt-2'>
            <h4 className='invoice-title'>
              Payout <span className='invoice-number text-primary'>#{data.num}</span>
            </h4>
      ...
      <CardBody className='invoice-padding pt-0'>
        <Row className='invoice-spacing'>
          <Col className='p-0' lg='8'>
            <h6 className='mb-2'>Payout To:</h6>
            <h6 className='mb-25'>{data.curator_vs_campaign.curator.name}</h6>
            <CardText className='mb-25'>{data.curator_vs_campaign.curator.phone}</CardText>
            <CardText className='mb-25'>{data.curator_vs_campaign.curator.email}</CardText>
            ...
            <table>
              <tbody>
                <tr>
                  ...
                  <td>
                    <span className='font-weight-bolder'>{data.curator_vs_campaign.campaign.name}</span>
                  </td>
                </tr>
                <tr>
                  <td className='pr-1'>Language:</td>
                  <td>
                    {data.curator_vs_campaign.campaign.language.name}
                  </td>
                </tr>
                <tr>
                  <td className='pr-1'>Publish Date:</td>
                  <td>
                    {moment(data.curator_vs_campaign.campaign.start_date).format('MMMM, DD-Y')}
                  </td>
                </tr>
                <tr>
                  <td className='pr-1'>Genres:</td>
                  <td>
                    {_.map(data.curator_vs_campaign.campaign.genres, (x, i) => {
                      return (
                          <Badge className={'mx-50'} key={i} pill color={'primary'}>
                            {x.name}
                          </Badge>
                      )
                    })}
            ...
            <Row className='invoice-spacing'>
              <Col className='p-0 mt-xl-0 mt-2' lg='12'>
                ...
                <table>
                  ...
                  {console.log(data.attachment_link ? 'true' : 'false')}
                  {
