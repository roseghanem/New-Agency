...
import {
    _getAllAdminsWithQ
} from "@modules/user/redux/actions"
import AsyncSelect from "react-select/async"
import PasswordStrengthBar from "react-password-strength-bar"

const AdminVsCampaignModal = (props) => {
                ...
                <ModalBody className='flex-grow-1 pb-sm-0 pb-3 modal-body'>
                    <FormGroup>
                        <Label className='form-label' for={`admin_ids`}>
                            Select your Manager <small className={'text-danger'}>*</small>
                        </Label>
                        <Controller
                            as={AsyncSelect}
                            control={control}
                            ...
                            defaultValue={_.get(props, 'data.admins') ? _.map(props.data.admins, x => ({label: x.name, value: x.id})) : ''}
                            rules={{
                                required: true
                            }}
                        />
...
export default AdminVsCampaignModal
