...
import {_logout} from '@authModule'
...
//************************************//
export const trans = (id, vars = {}) => {
  const msg = _.get(_getGlobal(`messages`), id, id)
  return msg === id ? msg : new IntlMessageFormat(msg).format(vars)
  // return (<FormattedMessage id={id} values={{...vars}} defaultMessage={id}/>)
}

//************************************//
export const navTrans = (id, vars = {}) => {
  return (<FormattedMessage id={id} values={{...vars}} defaultMessage={id}/>)
}
...
export const _confirm = (opt = {}) => {
  const {
    title = trans('gen.confirmModal.title'),
    text = trans('gen.confirmModal.text'),
    icon = 'warning',
    confirmButtonText = trans('gen.confirmModal.confirmButtonText'),
    cancelButtonText = trans('gen.actions.cancel'),
    width = null
  } = opt
  return MySwal.fire({
    title,
    ...
    if (result.value) {
      if (opt.callback) {
        opt.callback(() => {
          MySwal.fire({
            icon: 'success',
            title: trans('gen.confirmModal.callBackTitle'),
            text: '',
            customClass: {
...
export function _setAPILang(lang) {
    ...
    _.forEach(PluggedInModules, (module, _i) => {
        if (module.path.Axios) {
            _.forEach(module.path.Axios, (API, _i) => {
                module.path.Axios[_i].defaults.headers['lang'] = lang
            })
        }
    })
}
...
//************************************//
export const _getDateTimeFromInputWithFormat = (val, format) => {
    let finalValue = val
    if (typeof val === 'string') {
        finalValue = val
    } else if (moment(val[0]).format(format) !== 'Invalid date') {
        finalValue = moment(val[0]).format(format)
    } else {
        finalValue = ''
    }
    return finalValue
