...
import {_url} from "../../../../modules/core/utility/Utils"
...
const UserDropdown = () => {
  ...
  const user = useSelector(store => store.artist.userData)
  //** Vars
  //const userAvatar = (user && user.image) || defaultAvatar
  return (
    <UncontrolledDropdown tag='li' className='dropdown-user nav-item'>
      <DropdownToggle href='/' tag='a' className='nav-link dropdown-user-link' onClick={e => e.preventDefault()}>
        ...
        {/*<Avatar imgHeight='40' imgWidth='40' status='online' content={user.name ?? ''} initials />*/}
          <CustomAvatar img={_url(user.image)} content={user.name ?? ''} imgHeight='40' imgWidth='40' status='online' />
      </DropdownToggle>
      <DropdownMenu right>
